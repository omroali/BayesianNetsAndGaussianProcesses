...
class BayesNetInference(BayesNetReader):
    def __init__(self, alg_name, file_name, prob_query, num_samples):
        ...
        if alg_name == "InferenceByEnumeration":
            ...
            print(f"-\nRejection-sampling-{num_samples}:")
...
if __name__ == "__main__":
    if len(sys.argv) < 4 or len(sys.argv) > 5:
        print(
            "USAGE: BayesNetInference.py [inference_algorithm] [your_config_file.txt] [query] (num_samples)"
        )
        print(
            'EXAMPLE1> BayesNetInference.py InferenceByEnumeration config-alarm.txt "P(B|J=true,M=true)"'
        )
        print(
            'EXAMPLE2> BayesNetInference.py RejectionSampling config-alarm.txt "P(B|J=true,M=true)" 10000'
        )
        exit(0)

    alg_name = sys.argv[
        1
    ]  # inference algorithm={InferenceByEnumeration,RejectionSampling}
