...
class Edge:
    ...
    def is_independent(self, var_i: str, var_j: str, parents: list[str]) -> bool:
        return self.independence_test(
            self.conditional_independence_test,
            var_i,
            var_j,
            parents,
    ...
    def is_dependent(self, var_i: str, var_j: str, parents: list[str]) -> bool:
        return not self.independence_test(
            self.conditional_independence_test,
            var_i,
            var_j,
            parents,
    ...
    def markov_approved(self) -> bool:
        ...
        """
        return (
            self.is_conditionally_independent
            and self.has_minimality
            and self.has_faithfulness
        )
    ...
    def is_conditionally_independent(self) -> bool:
        ...
        self.test_value = float(
            self.conditional_independence_test.compute_pvalue(
                self.var_i, self.var_j, self.parents
            )
        )
    ...
    def has_minimality(self) -> bool:
        ...
        if self.is_multi_parent:
            raise ValueError(
                "ERROR: Only one parent is currently support for current minimality test"
            )
    ...
    def independence_test(
        ci: ci, var_i: str, var_j: str, parents: list[str], threshold: float
    ) -> bool:
